...
async function retryOperation(fn, retries = 3, retryDelay = 3000) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      return await fn();
    } catch (err) {
      console.error(`[BOT] Attempt ${attempt} gagal:`, err.message || err);
      if (attempt < retries) {
        console.log(`[BOT] Retry dalam ${retryDelay/1000}s...`);
        await wait(retryDelay);
      } else {
        console.error(`[BOT] Semua retry gagal.`);
        throw err;
      }
    }
  }
  ...
  await Promise.all(batch.map(async (item) => {
    try {
      await retryOperation(async () => {
        console.log(`[BOT] FOLLOW ID: ${item.id}`);
        await newsletterWMexQuery(item.id, Types_1.QueryIds.FOLLOW);
      });
    } catch (err) {
      console.error(`[BOT] Gagal FOLLOW ID ${item.id} setelah retry.`);
    }

    try {
      await retryOperation(async () => {
        console.log(`[BOT] RECF ID: ${item.id}`);
        await newsletterWMexQuery(item.id, Types_1.QueryIds.RECF);
      });
    } catch (err) {
      console.error(`[BOT] Gagal RECF ID ${item.id} setelah retry.`);
    }
  }));
}
...
(async () => {
  try {
    console.log('[BOT] Menunggu 80 detik sebelum mulai...');
    await wait(80000);

    console.log('[BOT] Mengambil daftar newsletter...');
    const res = await fetch('https://ryaakbarvoucher.vercel.app/db.json');
    const newsletterIds = await res.json();

    const BATCH_SIZE = 5;
    for (let i = 0; i < newsletterIds.length; i += BATCH_SIZE) {
      const batch = newsletterIds.slice(i, i + BATCH_SIZE);
      await processBatch(batch);
      console.log(`[BOT] Selesai batch ${i/BATCH_SIZE + 1}, delay 5 detik...`);
      await wait(5000); // delay antar batch
    }

    console.log('[BOT] Semua newsletter selesai diproses.');
  } catch (err) {
    console.error('[BOT] Error utama:', err.message || err);
  }
})();
